...
def reset_robot():
    ...
    cmd_topic = '/sim_p3at/cmd_vel' # Senin robotunun hareket topic'i
    
    # --- AYARLAR: BASLANGIC NOKTASI ---
    start_x = -20.799
    start_y = 14.3
    start_z = 0.03
    start_yaw = 0.0
    ...
    # 1. Once Motorlari Durdur (El Freni)
    # Robot isinlanmadan once ve sonra hizini sifirlamak onemlidir.
    ...
    for _ in range(10): # 10 kere ust uste DUR emri gonder
        pub.publish(stop_msg)
        time.sleep(0.05)

    # 2. Robotu Isinla (Teleport)
    ...
    # 3. Isinlandiktan Sonra Tekrar Durdur (Guvenlik Icin)
    rospy.loginfo("Yeni konumda sabitleniyor...")
    for _ in range(20): # Isinlandiktan sonra da frene basmaya devam et
        pub.publish(stop_msg)
        time.sleep(0.05)
